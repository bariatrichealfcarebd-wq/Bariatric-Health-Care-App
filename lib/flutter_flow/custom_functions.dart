...
// Se a senha for nula ou vazia, retorna falso
  if (senha == null || senha.isEmpty) {
    return false;
  }

  // Retorna Verdadeiro se tiver 8 ou mais caracteres
  return senha.length >= 8;
}

List<dynamic> processarGraficoPeso(List<PacientePesoRecord>? docs) {
// Proteção contra lista nula
  ...
  for (var doc in docs) {
    // O SEGREDO: Só adiciona se TIVER DATA *E* TIVER PESO
    if (doc.peso > 0) {
      resultado.add({
        // Já formata a data para texto (Dia/Mês) aqui mesmo
        'data': DateFormat('dd/MM').format(doc.dataRegistro!),
        'peso': doc.peso,
      });
    }
  }
